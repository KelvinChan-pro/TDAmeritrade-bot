...
void TDA::disp_Trading() {
    ...
    std::cout << "ORDER_CODE = " << ORDER_CODE << std:: endl;
        ...
        if (!existFlag) {
            if (this->account.account_type() == "CASH") {
                if (this->account.projected_cash_available_for_trading() + EPS < (price - PRICEGAP * (i + 1)) * QUANTITY) {
                    ...
                    rlt = this->account.placeOrder(ORDER_CODE, TICKER, price - PRICEGAP * (i + 1), PROFIT, QUANTITY, STOPPRICEOFFSET);
                    if( rlt == "Request Error!" || rlt == "Refresh Token!") {
                        std::cout << rlt << std::endl;
                        return;
                    }
...
void TDA::place_Orders() {
    ...
    for (int i = 0; i < ORDER_NUM; i++) {
        this->account.placeOTOTAOrder(TICKER, price - PRICEGAP * (i + 1), PROFIT, QUANTITY, STOPPRICEOFFSET);
    }
}
