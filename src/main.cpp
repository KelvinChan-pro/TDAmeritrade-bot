...
int tickCount = 0;
int tickCount_Memory = -1;
boolean tradingFlag = true;
std::thread *pThread;
...
void accountThread(TDA *tda) {
    while(true) {
        if(tickCount == tickCount_Memory) {
            tradingFlag = false;
            std::cout << "tickCount = " << tickCount << " tickCount_Memory = " << tickCount_Memory << " Trading Thread stopped, please restart" << std::endl;
            tradingFlag = true;
            pThread->join();
        } else {
            tradingFlag = true;
            std::cout << "tickCount = " << tickCount << " tickCount_Memory = " << tickCount_Memory <<  " Trading Thread Normal working" << std::endl;
        }
        tickCount_Memory = tickCount;
        sleep(THREAD_CYCLING_TIME_ACCOUNT);
    }
}

void tradingThread(TDA *tda) {
    while(true) {
        if(!tradingFlag) break;
        std::time_t t = std::time(0);   // get time now
        std::tm* now = std::localtime(&t);
        tickCount = now->tm_min * 60 + now->tm_sec;
